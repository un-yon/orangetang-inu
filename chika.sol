...
contract Chika is Ownable {
    string private constant NAME =  "Chika";
    string private constant SYMBOL = "$CHIKA";
    ...
    function setFees(uint8 newBuyFee, uint8 newSellFee) external onlyOwner {
        require(newBuyFee <= 10 && newSellFee <= 10, "fees must be <=10%");
        require(newBuyFee != _buyFee || newSellFee != _sellFee, "new fees cannot be the same as old fees");
        _buyFee = newBuyFee;
        _sellFee = newSellFee;
    }
